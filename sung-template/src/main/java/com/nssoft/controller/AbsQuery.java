...
import com.nssoft.util.BeanMapConvertUtil;
 ...
 *
 * @ClassName AbsQuery
 * @Description 继承于AbsAPI
 * Copyright (c) 2014 by Ns Soft.  
 * @author xuliguo   
 * @date 2014年12月22日 下午5:34:33    
 * @version V1.0      
 *              
 */
...
private static final Log logger = LogFactory.getLog( AbsQuery.class );
	...
	public <T> T MapToBean(Class<T> clazz, Map map) {
		return BeanMapConvertUtil.toBean(clazz, map);
	}
	
	public void MapToBean(Object bean, Map map) {
		try {
	...
	/*public List selectList(Map paramMap, String queryString, SqlSession session) throws SQLException{
		return session.selectList(queryString, paramMap);
	}
	
	public T selectOne(Map paramMap, String queryString, SqlSession session) throws SQLException{
		return session.selectOne(queryString, paramMap);
	}
	
	public T selectOne(String paramString, String queryString, SqlSession session) throws SQLException{
		return session.selectOne(queryString, paramString);
	}*/
		...
		for (Object p : mEnumOptionParamClass.getEnumConstants()) {
			if (logger.isDebugEnabled()) {
				...
				if (value == null || value[0].length() == 0) {
					...
					//throw new InvalidMustParameterException("可选参数为空[" + p.toString() + "]");
