...
import com.nssoft.model.OpenApiUser;
import com.nssoft.util.BeanMapConvertUtil;
import com.nssoft.util.MD5Util;
/**   
 *
 * @ClassName OpenAPIQuery
 * @Description
 * Copyright (c) 2014 by NS Soft. 
 * @author xuliguo   
 * @date 2015年8月10日 下午3:50:20    
 * @version V1.0      
 *    
 */
public abstract class OpenApiQuery<T> extends AbsAPI<T> {
	
	private static final Logger logger = Logger.getLogger(OpenApiQuery.class);
	...
	public void checkAuth(HttpServletRequest req, HttpServletResponse res)
			throws AbsException {
		//super.checkAuth(req, res);
		Map paramMap = getParameterMap(req);
		OpenApiUser apiUser = MapToBean(OpenApiUser.class, paramMap);
