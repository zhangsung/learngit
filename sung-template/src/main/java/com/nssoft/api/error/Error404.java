...
@Controller("/v1/api/error/error404.json")
public class Error404 extends AbsQuery<String> {

	@Override
	public String getApiId() {
		return "error_error404";
	}
	...
	public String executeAPI(HttpServletRequest req, HttpServletResponse res,
		...
		if (isAjaxRequest) {
			throw new ApiException("未找到资源.", -100);
		} else {
			try {
				res.sendRedirect(req.getContextPath() + "/error/error404.html");
			} catch (IOException e) {
				e.printStackTrace();
			}
			return null;
